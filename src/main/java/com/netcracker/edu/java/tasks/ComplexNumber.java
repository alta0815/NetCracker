...
/**
 * GOALS<ul>
 * <li>To learn OOP basics: basic principles of writing a class, getters/setters.</li>
 * <li>To get familiar with some methods of {@link Object} and with {@link Comparable} interface.</li>
 * <li>To be able to use some java.lang and java.util API: {@link String}, {@link Double}, {@link Arrays}.</li>
 * </ul><p/>
 * TASK<br/>
 * Implement a class storing a single complex number with basic mathematical operations on it.
 * Instances of the class must be comparable, cloneable and representable as String (in the common format).<br/>
 * <p/>
 * REQUIREMENTS<br/>
 * - The complex number consists of two components: real part (re) and imaginary part (im).
 *   They should be stored as private fields (members of the class) of type double or Double.
 *   After instantiation of the number with default constructor both re and im must be equal to 0.<br/>
 * - Every math operation on the number (add, negate, multiply methods) should change the state of this object 
 *   not creating a new object (this requirement is intended to save memory and to lower calculation time). 
 *   For a caller's convenience, all these methods (math operations) return this object itself. <br/>
 * - The common requirement for any task (to check it online) is to declare your class as follows:<br/>
 * <code>public class ComplexNumberImpl implements ComplexNumber {  } </code><br/>
 * <p/>
 * NOTES<br/>
 * - When coded with the methods of this interface, formulas like z = x-y^2 could look like this:
 *  <code>ComplexNumber z = x.add(y.copy().multiply(y).negate());</code> 
 *  where <code>x</code>, <code>y</code> are objects of type ComplexNumber.<br/>
 * - The task can be solved without any knowledge of Exceptions, but if you know that topic 
 *  explicit generation of {@link NumberFormatException} in {@link #set(String)} method is recommended.
 * - It's recommended (but not required) to have the following set of constructors:<br/>
 *   <code>public ComplexNumberImpl(double re, double im);</code><br/>
 *   <code>public ComplexNumberImpl(String value);</code><br/>
 *   <code>public ComplexNumberImpl();</code> (this default constructor is required by the testing system).<br/>
 * - If you don't understand the mathematical meaning of the complex number you can read e.g. 
 *  <a href="http://en.wikipedia.org/wiki/Complex_number">Complex Number at Wikipedia</a>.<br/>
 * - A professional Java implementation of this concept can be found here:
 *  <a href="http://commons.apache.org/proper/commons-math/userguide/complex.html">Complex in Apache Commons Math</a>.
 * <p/>
...
//Getters and setters
     ...
     *   but NumberFormatException can be thrown by {@link Double#parseDouble(String)}).
     * @param value
     * @throws NumberFormatException if the given string value is incorrect
     */
    void set(String value) throws NumberFormatException;

//Methods of java.lang.Object an similar methods
    /**
    ...
    /**
     * Returns a string representation of this number, that must be compatible with {@link #set(String)}:
...
//Ordering methods
     ...
     *  e.g. x < y if and only if |x| < |y| where |x| denotes absolute value (modulus) of x.<br/>
     * The square of the absolute value of a complex number is defined as the sum of
     *  the real part squared and the imaginary part squared: |re+imi|^2 = re^2 + im^2
